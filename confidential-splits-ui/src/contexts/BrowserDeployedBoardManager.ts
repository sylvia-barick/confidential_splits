    ...
    fnPipe(
      ...
      timeout({
        ...
        with: () =>
          throwError(() => {
            ...
            return new Error('Could not find Midnight wallet. Extension installed?');
          }),
      }),
      concatMap(async (initialAPI) => {
        const connectedAPI = await getSharedWalletConnection(() => initialAPI.connect(networkId));
      ...
      catchError((error: unknown) =>
        throwError(() => {
          ...
          return new Error(`Wallet connection failed: ${reason}. Please try again.`);
        }),
      ),
    ),
  );
