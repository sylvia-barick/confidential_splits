...
import { getSharedWalletConnection, withHostedWalletRetry } from './walletConnection';
...
export class BrowserDeployedSplitsManager implements DeployedSplitsAPIProvider {
  ...
  resolve(contractAddress?: ContractAddress, initialMembers?: Uint8Array[]): Observable<SplitsDeployment> {
    const deployments = this.#splitsDeploymentsSubject.value.filter((d) => d.value.status !== 'failed');
  ...
  return {
    ...
    midnightProvider: {
      submitTx: async (tx: FinalizedTransaction): Promise<TransactionId> => {
        const serializedTx = toHex(tx.serialize());
        try {
          await withHostedWalletRetry(() => connectedAPI.submitTransaction(serializedTx));
        } catch (e) {
          logger.error({ error: e }, 'Error submitting Splits transaction via wallet');
          const reason = e instanceof Error ? e.message : String(e);
          throw new Error(
            `Failed to submit transaction: ${reason}. This is usually a transient issue with the 1AM wallet's hosted service - please try again.`,
          );
        }
...
const connectToWallet = (logger: Logger, networkId: string): Promise<ConnectedAPI> => {
  return firstValueFrom(
    fnPipe(
      ...
      map((connectorAPI) => getSharedWalletConnection(() => connectorAPI.connect(networkId))),
      concatMap((promise) => from(promise)),
      // The wallet's connect handshake is a network round-trip to hosted 1AM infrastructure and
      // can fail transiently (mirrors the hosted prover's intermittent /check 500s). Retry once
      // before giving up.
      retry({ count: 1, delay: 750 }),
      catchError((error: unknown) =>
        throwError(() => {
          const reason = error instanceof Error ? error.message : String(error);
          logger.error({ error }, 'Wallet connect handshake failed for Splits contract');
          return new Error(
            `Wallet connection failed: ${reason}. This is usually a transient issue with the 1AM wallet's hosted service - please try again.`,
          );
        }),
      ),
    ),
