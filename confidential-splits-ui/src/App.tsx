...
import { getSharedWalletConnection } from './contexts/walletConnection';
...
// Deploying/joining involves: wallet handshake, ZK proof generation, wallet approval + balancing,
// broadcast, and then waiting for the Preprod indexer to observe the transaction in a block. Each
// of those steps can individually take many seconds on a public testnet, so a short timeout here
// trips even on a perfectly healthy, if slightly slow, end-to-end run.
...
const waitForDeployment = (deployment$: Observable<SplitsDeployment>): Promise<DeployedSplitsDeployment> => {
  return new Promise((resolve, reject) => {
    const timer = setTimeout(() => {
      ...
      reject(
        new Error(
          `Contract resolution timed out after ${DEPLOYMENT_TIMEOUT_MS / 1000}s. Proof generation, wallet approval, and Preprod block confirmation can be slow - please check the contract address and network connection, then try again.`,
        ),
      );
...
const App: React.FC = () => {
  ...
  const [isConnectingGroup, setIsConnectingGroup] = useState<boolean>(false);
  ...
  const connectOneAm = async (): Promise<void> => {
    ...
    try {
      ...
      const connectedApi = await getSharedWalletConnection(() => oneAm.connect('preprod'));
  ...
  const handleConnectToGroup = async (addressToConnect?: string): Promise<void> => {
    const targetAddr = (addressToConnect || splitsAddressInput).trim().replace(/^0x/i, '');
    if (!targetAddr) {
      setTxError('Please enter a contract address to connect');
      return;
    }
    if (!/^[0-9a-fA-F]{64}$/.test(targetAddr)) {
      setTxError(
        `Invalid contract address: "${targetAddr}". A Midnight contract address must be exactly 64 hexadecimal characters (32 bytes).`,
      );
      return;
    }
  ...
  const handlePostExpense = async (): Promise<void> => {
    ...
    try {
      ...
      // Ensure caller is the payer and active key is loaded
      const payerIdx = currentUserIdx;
      const secretKey = await hashAddressToSecretKey(walletAddress);
      await splitsAPI.changeActiveUser(payerIdx, secretKey);
      ...
      setTxStage('AWAITING_WALLET');
      await splitsAPI.postExpense(BigInt(payerIdx), amountVal, shares);
      setTxStage('CONFIRMED');
    } catch (e: unknown) {
  ...
  const handleSyncBalance = async (): Promise<void> => {
    ...
    try {
      ...
      setTxStage('PROVING');
      const secretKey = await hashAddressToSecretKey(walletAddress);
      await splitsAPI.changeActiveUser(currentUserIdx, secretKey);
  ...
  const handlePostPayment = async (debtorIdx: number, creditorIdx: number, amount: bigint): Promise<void> => {
    ...
    try {
      ...
      setTxStage('PROVING');
      const secretKey = await hashAddressToSecretKey(walletAddress);
      await splitsAPI.changeActiveUser(debtorIdx, secretKey);
  ...
  const handleClaimPayment = async (): Promise<void> => {
    ...
    try {
      setNetworkId('preprod');
      setTxStage('PROVING');
      const creditorIdx = Number(ledgerState.pending_payment_to);
      const secretKey = await hashAddressToSecretKey(walletAddress);
      await splitsAPI.changeActiveUser(creditorIdx, secretKey);
  ...
  // Keep expense payer synchronized with current user
  useEffect(() => {
    if (currentUserIdx !== null) {
      setExpensePayer(currentUserIdx);
    }
  }, [currentUserIdx]);

  // Handle auto-detect role on ledger or wallet state changes.
  //
  // `ledgerState` is replaced with a new object on every `state$` emission, including the ones
  // `splitsAPI.changeActiveUser()` itself causes (it feeds into `state$` via `combineLatest`). If
  // this effect called `changeActiveUser` unconditionally, every emission would trigger another
  // write, which would trigger another emission, forever - freezing the tab in a synchronous
  // feedback loop. Skipping the write once we're already resolved to the right slot breaks the
  // cycle.
  useEffect(() => {
    const resolveConnectedUser = async () => {
      if (!walletConnected || !walletAddress || !ledgerState) {
        return;
      }
      try {
        const secretKey = await hashAddressToSecretKey(walletAddress);
  ...
  }, [walletConnected, walletAddress, ledgerState, splitsAPI, currentUserIdx]);
  ...
  const allSynced = ledgerState
    ? ledgerState.members.every((m, i) => m === EMPTY_MEMBER || ledgerState.synced_mask[i])
    : false;
                ...
                <Box component="ol" sx={{ m: 0, pl: 2.25, '& li': { mb: 0.35, fontSize: '0.8rem' } }}>
                  ...
                  <li>
                    ...
                    <Box component="code" sx={{ fontFamily: "'JetBrains Mono', monospace", fontSize: '0.75rem' }}>
                      docker compose -f confidential-splits-cli/proof-server-local.yml up -d
                    </Box>
                  </li>
                ...
                {latestContractAddress ? (
                  ...
                  <Box sx={{ display: 'flex', gap: 2, flexWrap: 'wrap', alignItems: 'flex-start' }}>
                    <Box sx={{ display: 'flex', gap: 1.5, flexWrap: 'wrap', flex: '1 1 320px' }}>
                      ...
                      <Button
                        variant="outlined"
                        onClick={() => void handleConnectToGroup()}
                        disabled={isConnectingGroup}
                      >
                  ...
                  >
                    <Stack spacing={2} sx={{ flexGrow: 1 }}>
                      ...
                      <Button
                        ...
                        disabled={currentUserIdx === null || !allSynced}
                        fullWidth
                      >
                        Post split expense
                      </Button>
