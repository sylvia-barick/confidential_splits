 ...
 * exact same `SplitsAPI` (`api/src/splits-api.ts`) that `confidential-splits-ui/src/App.tsx` uses
...
function loadResults(): { contractAddress?: string; transactions: TxRecord[] } {
  if (fs.existsSync(OUT_PATH)) {
    try {
      const parsed = JSON.parse(fs.readFileSync(OUT_PATH, 'utf-8')) as {
...
const hasStep = (step: string, wallet: 'A' | 'B'): boolean =>
  results.some((r) => r.step === step && r.wallet === wallet);

/** Byte-for-byte port of `hashAddressToSecretKey` in confidential-splits-ui/src/App.tsx (SHA-256 of the UTF-8 address). */
...
let contractAddress: ContractAddress | undefined = loaded.contractAddress;
...
async function startEnvWithRetry(
  ...
  testEnv: ReturnType<Config['getEnvironment']>;
  env: Awaited<ReturnType<ReturnType<Config['getEnvironment']>['start']>>;
}> {
  ...
  for (let i = 1; i <= attempts; i++) {
    ...
    try {
      ...
      logger.warn(
        `[${role}] testEnv.start() attempt ${i}/${attempts} failed: ${message}. Tearing down and retrying...`,
      );
  ...
  throw lastError instanceof Error
    ? lastError
    : new Error(`[${role}] testEnv.start() failed after ${attempts} attempts`);
}

async function buildFundedWallet(
  role: 'A' | 'B',
  seed: string,
): Promise<{ provider: MidnightWalletProvider; address: string }> {
...
function makeProviders(walletProvider: MidnightWalletProvider, storeSuffix: string, seed: string): SplitsProviders {
  const zkConfigProvider = new NodeZkConfigProvider(
    path.resolve(currentDir, '..', '..', 'contract', 'src', 'managed', 'splits'),
  );
  return {
...
async function main(): Promise<void> {
  ...
  try {
    ...
    if (!hasStep('post_expense', 'A')) {
      ...
      const postExpenseTx = await apiA.deployedContract.callTx.post_expense(0n, 1200n, [600n, 600n, 0n, 0n]);
    ...
    if (!hasStep('sync_balance', 'A')) {
      ...
      if (!walletState.saltAHex)
        throw new Error('sync_balance(A) was recorded but its salt was not persisted - cannot resume safely.');
    ...
    if (!hasStep('sync_balance', 'B')) {
      ...
      if (!walletState.saltBHex)
        throw new Error('sync_balance(B) was recorded but its salt was not persisted - cannot resume safely.');
    ...
    if (!hasStep('post_payment', 'B')) {
      ...
      const postPaymentTx = await apiB.deployedContract.callTx.post_payment(
        1n,
        0n,
        600n,
        balanceB,
    ...
    writeState('complete');
    logger.info(
      '=== Splits Preprod E2E complete. All 7 contract transactions are real, on-chain, and independently verifiable. ===',
    );
  } catch (e) {
