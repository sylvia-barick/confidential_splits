 ...
 *   OUTPUT_FILE             resumable JSON checkpoint (default: confidential-splits-cli/preprod-address-activity.json)
 *   ADDRESSES_CSV           CSV export path (default: confidential-splits-cli/preprod-addresses.csv)
 *   ADDRESSES_TXT           plain address-per-line export path (default: confidential-splits-cli/preprod-addresses.txt)
 *   VERIFICATION_FILE       sample verification queries for the top addresses
 *                           (default: confidential-splits-cli/preprod-addresses-verification.txt)
  ...
  while (attempt < MAX_RETRIES) {
    ...
    try {
      ...
      const backoffMs = isRateLimited
        ? Math.min(30_000 * 2 ** attempt, 180_000)
...
function verificationQuery(txHash: string): string {
  ...
  return `curl -s -X POST ${INDEXER_URL} -H 'content-type: application/json' ` + `-d '${JSON.stringify({ query })}'`;
}
...
async function main(): Promise<void> {
  const logger = await createLogger(
    path.resolve(currentDir, '..', 'logs', 'scan-preprod-addresses', `${new Date().toISOString()}.log`),
  );
  ...
  if (process.env.START_HEIGHT !== undefined) {
    ...
    startHeight = await resolveLookbackStartHeight(
      latestHeight,
      latest.timestampMs,
  ...
  if (STOP_AT_ADDRESS_COUNT > 0) {
    logger.info(
      `STOP_AT_ADDRESS_COUNT=${STOP_AT_ADDRESS_COUNT}: will stop as soon as that many distinct addresses are observed.`,
    );
  }
  ...
  if (alreadyAtTarget) {
    logger.info(
      `Checkpoint already has ${Object.keys(addresses).length} addresses, meeting STOP_AT_ADDRESS_COUNT=${STOP_AT_ADDRESS_COUNT}. Skipping scan.`,
    );
  ...
  process.on('SIGINT', () => {
    ...
    logger.info(
      `Checkpoint saved to ${OUTPUT_FILE}. Re-run the same command to resume from height ${lastScannedHeight + 1}.`,
    );
    process.exit(130);
  });
  ...
  if (Object.keys(addresses).length < 50) {
    logger.warn(
      `Only ${Object.keys(addresses).length} distinct addresses were verified in the scanned range - fewer than 50.`,
    );
  }
  logger.info(
    `Unique contract addresses interacted with: ${uniqueContractAddresses.size} (excluded from address export)`,
  );
